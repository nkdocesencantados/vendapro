...
export default function InventoryPage() {
  ...
  const [form, setForm] = useState({ name: "", description: "", price: "", costPrice: "", stock: "", minStock: "", category: "" })
        ...
        {showForm && (
          <div style={{ background: "white", border: "0.5px solid #e5e7eb", borderRadius: "12px", padding: "20px", marginBottom: "20px" }}>
            ...
            <div className="inv-form-grid">
              {([["Nome *","name","text"],["Categoria","category","text"],["Preco de venda *","price","number"],["Custo","costPrice","number"],["Estoque inicial","stock","number"],["Estoque minimo","minStock","number"]] as [string,string,string][]).map(([label,field,type]) => (
                <div key={field}>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px" }}>
              ...
              <button onClick={saveProduct} disabled={saving} style={{ padding: "8px 16px", background: saving ? "#9ca3af" : "#1D9E75", color: "white", border: "none", borderRadius: "8px", cursor: saving ? "not-allowed" : "pointer", fontSize: "13px" }}>{saving ? "Salvando..." : "Salvar"}</button>
            </div>
          </div>
        )}
        {loading ? (
              ...
              <div key={p.id} style={{ background: "white", border: p.stock <= p.minStock ? "1px solid #f59e0b" : "0.5px solid #e5e7eb", borderRadius: "10px", padding: "14px" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", gap: "8px" }}>
                  <div>
                    ...
                    <div style={{ fontWeight: 600, color: "#1D9E75", fontSize: "13px" }}>{fmt(p.price)}</div>
                  </div>
