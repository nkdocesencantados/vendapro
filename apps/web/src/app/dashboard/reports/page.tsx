...
export default function ReportsPage() {
  ...
  async function exportExcel() {
    try {
      ...
      const d = r.data
      const rows = [
        ["Relatorio VendaPro - " + new Date().toLocaleDateString("pt-BR")],
        [],
        ["RESUMO"],
        ["Total de Vendas", fmt(d.totalRevenue)],
        ["Qtd de Vendas", d.totalSales],
        ["Ticket Medio", fmt(d.avgTicket)],
        ["Lucro Estimado", fmt(d.estimatedProfit)],
        [],
        ["PRODUTOS MAIS VENDIDOS"],
        ["Produto", "Qtd Vendida", "Faturamento"],
        ...(d.topProducts || []).map((p: any) => [p.name, p.quantity, fmt(p.revenue)]),
        [],
        ["VENDAS POR DIA"],
        ["Data", "Valor"],
        ...(d.dailyChart || []).map((x: any) => [x.day, fmt(x.value)]),
      ]
      const csv = rows.map(r => r.join(";")).join("\n")
      const blob = new Blob(["\uFEFF" + csv], { type: "text/csv;charset=utf-8;" })
      const url = URL.createObjectURL(blob)
      const a = document.createElement("a"); a.href = url
      a.download = `relatorio-vendapro-${from}.csv`
  ...
  if (loading) return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ background: "white", borderBottom: "0.5px solid #e5e7eb", padding: "0 20px", height: "50px", display: "flex", alignItems: "center" }}>
        <div style={{ fontSize: "14px", fontWeight: 500 }}>Relatorios</div>
      </div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", flex: 1 }}>
        <div style={{ color: "#888" }}>Carregando dados...</div>
      </div>
    </div>
  )
  ...
  const categoryChart = d.categoryChart || []
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ background: "white", borderBottom: "0.5px solid #e5e7eb", padding: "0 20px", height: "50px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", gap: "8px" }}>
          {[["week", "7 dias"], ["month", "Este mes"], ["year", "Este ano"]].map(([v, l]) => (
            <button key={v} onClick={() => setPeriod(v)} style={{ padding: "5px 12px", fontSize: "12px", border: "0.5px solid #e5e7eb", borderRadius: "6px", cursor: "pointer", background: period === v ? "#1D9E75" : "white", color: period === v ? "white" : "#666" }}>{l}</button>
          ))}
          <button onClick={exportExcel} style={{ padding: "5px 12px", fontSize: "12px", border: "0.5px solid #e5e7eb", borderRadius: "6px", cursor: "pointer", background: "white", color: "#666" }}>Exportar CSV</button>
          <button onClick={printReport} style={{ padding: "5px 12px", fontSize: "12px", border: "0.5px solid #e5e7eb", borderRadius: "6px", cursor: "pointer", background: "white", color: "#666" }}>Imprimir</button>
        </div>
      </div>
          ...
          <>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "12px", marginBottom: "20px" }}>
              {[
                ...
                { label: "Produtos Parados", value: d.slowProducts || 0, color: "#f59e0b", sub: "sem movimento" },
              ].map(m => (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
              <div style={{ background: "white", border: "0.5px solid #e5e7eb", borderRadius: "12px", padding: "16px" }}>
                ...
                {dailyChart.slice(-14).map((x: any) => (
                  <div key={x.day} style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "8px" }}>
                    <div style={{ fontSize: "11px", color: "#888", width: "40px", flexShrink: 0 }}>{x.day}</div>
                    <Bar value={x.value} max={maxDaily} />
                    <div style={{ fontSize: "11px", color: "#333", width: "70px", textAlign: "right" }}>{fmt(x.value)}</div>
                  </div>
                ))}
              </div>
            ...
            {topProducts.map((p: any) => (
              <div key={p.name} style={{ display: "grid", gridTemplateColumns: "2fr 1fr 1fr 1fr", gap: "8px", padding: "10px 0", borderTop: "0.5px solid #f3f4f6", alignItems: "center" }}>
                <div>
                  ...
                  <div style={{ fontSize: "11px", color: "#888" }}>{p.category || "Sem categoria"}</div>
                </div>
                ...
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: "12px", color: "#666" }}>{d.totalRevenue > 0 ? Math.round((p.revenue / d.totalRevenue) * 100) : 0}%</div>
                  <Bar value={p.revenue} max={maxProduct} color="#3b82f6" />
                </div>
              </div>
            ))}
        ...
        {tab === "sales" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "12px" }}>
              {[
                { label: "Total de vendas", value: d.totalSales || 0, suffix: "vendas" },
