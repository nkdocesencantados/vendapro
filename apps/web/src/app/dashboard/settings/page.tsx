...
export default function SettingsPage() {
  ...
  useEffect(()=>{
    ...
    api.get("/stores").then(r=>{
      ...
      if(s){
        setName(s.name||""); setGoal(s.monthlyGoal?String(s.monthlyGoal):""); setMargin(String(s.margin||"26")); setAddress(s.address||""); setStoreId(s.id); if(s.phone)setPhone(s.phone); if(s.cnpj)setCnpj(s.cnpj)
