...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    api.get("/stores").then(r => {
      ...
      if (s) {
        ...
        if (s.trialExpired || s.status === 'inactive' || s.status === 'blocked') setTrialExpired(true)
      }
    }).catch(() => {})
  ...
  if (trialExpired) return (
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"var(--bg)",flexDirection:"column",gap:16,padding:24,textAlign:"center"}}>
      ...
      <h1 style={{fontSize:22,fontWeight:700,color:"var(--text)",margin:0}}>{trialDaysLeft===0?"Período de trial encerrado":"Acesso suspenso"}</h1>
      <p style={{fontSize:14,color:"var(--text-muted)",maxWidth:380,lineHeight:1.6,margin:0}}>{trialDaysLeft===0?"Seus 7 dias gratuitos chegaram ao fim. Para continuar usando o VendaPro, entre em contato para ativar seu plano.":"Sua conta foi suspensa. Entre em contato para regularizar o acesso."}</p>
